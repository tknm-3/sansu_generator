...
- [2026-06-13] 冒険/てじゅん(矢印ならべ)ゾーン3つ追加（🌈にじの おか・🍄きのこの もり・
  ...
  ・**やさしい→むずかしいは「ジェムの置き方」で段階づける**。① 単調経路（start左上→goal右下の
    対角線 (1,1)(2,2)(3,3)）に並べると optimal=マンハッタン最短のまま＝やさしい。② start左下→goal右上＋
    反対角線 (3,1)(2,2)(1,3) は「うえ→みぎ→うえ→みぎ」の かいだんを 組ませる（順序の学び）。
    ③ **ボスは ジェムを 真ん中(2,2)＋反対の すみっこ (0,4)/(4,0)/(0,5)/(5,0) に置く**と、どの順で
    まわるか＝巡回順の判断が要り optimal が はねあがる（5×5中央+2隅=16、6×6中央2つ+2隅=22）。
    「複雑で学習効果が高い」＝この巡回順の意思決定のこと。長くなるので maxSlots は optimal+4〜6。
  ・**落とし穴: `walls` を省略すると 盤面整合テストが `for (const w of q.walls)` で undefined を回して
    クラッシュ**する（「ばんめんが せいごう」が落ちる）。かべ無しでも **`walls: []` を必ず書く**。
  ・optimal は手計算せず、`npx vitest run src/lib/programming/adventure.test.ts` の
    `expected <実際> to be <設定値>` から 実測値を写す（隅ジェムの巡回は手計算がズレやすい）。
  ・新ゾーンは `ArrowAdventureUnit.tsx` の **`REGION_TINT[zoneId]` を追加**（`ACCENT` は
    既存キー violet/lime/amber 等を再利用すれば追加不要）。lakeの minCorners テストは lake限定なので
    新ゾーンにはかからない＝「L字ズル不可」を強制したいなら同テストを その zoneId へ広げる。
- [2026-06-14] 冒険/「階段じゃない ひねり系」矢印ゾーン5つ＋宝島の相対版2つ追加（adv-q188〜q222）。
  「階段は もう簡単」と言われたときの 非・階段ひねりの作り方:
  ・**遠回り(detour)/うずまき(spiral)で optimal>マンハッタンに するには、単調(下＆右だけ)経路を
    すべて 断つ**必要がある。スタート左上・ゴール右下なら、かべに すきまが あっても 単調に
    すり抜けられる＝optimal=Mのまま。**すきまを「ゴールと反対側」に置く**(例: 縦かべの すきまを
    最下段にして ゴールは右上)と、いちど 遠ざかって 戻る U字に なり optimal が はねあがる。
    スネーク(ジグザグ往復)は すきまを 左右こうごに して 強制する。
  ・**あともどり(backtrack)**: ジェムを ゴールと逆側(スタートの後ろ)に置くと、単調経路から はずれ
    optimal が M超に なる。「とおざかってから もどる」順序の学び。隅2つ+対角ゴールが 効く。
  ・**おばけ(固定ゾンビ)スラローム の罠**: ゾンビを 主対角線に ならべるだけだと **端を回るL字で
    全部よけられて optimal=M**＝ひねりに ならない(q205/q207で踏んだ)。**ゾンビも かべと同じく
    「すきまを 反対側に した 柵/スネーク」で 遠回りを 強制**する(fixedゾンビは solve()上 かべ同等)。
  ・**とじこめ部屋(vault)**: ジェムの まわり3方向を かべで囲い 入口1つの ポケットにする。
    隅/辺なら 1〜2枚で 足りる。**スタート/ゴールを 囲って 詰ませない**よう注意。
  ・**相対版(kind:'relative')は relSolution を 手書きしない**: 一時 vitest で `solveRelative(q)` を

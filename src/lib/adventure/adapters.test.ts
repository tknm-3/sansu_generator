...
describe('わり算バトルは わける まえの 山＋わけた 絵を 見せる（5の段まで）', () => {
  it('divide ビジュアルで、あまりなし・式つき（dividend=divisor*quotient）', () => {
    for (let seed = 1; seed <= 30; seed++) {
      const q = divisionToBattle(seededRng(seed));
      expect(q.visual?.kind).toBe('divide');
      if (q.visual?.kind !== 'divide') throw new Error('kind mismatch');
      const { dividend, divisor, quotient, remainder, equationText } = q.visual;
      expect(dividend).toBeGreaterThanOrEqual(4);
      expect(dividend).toBeLessThanOrEqual(25);
      // わけわけ どうくつは あまりなし（きれいに わけられる）
      expect(remainder).toBe(0);
      expect(dividend).toBe(divisor * quotient);
      // 「これが ○÷△ だ」と わかる 式が ついている（山の かずと そろう）
      expect(equationText).toBe(`${dividend} ÷ ${divisor}`);
    }
  });
});

...
// 難易度＝盤面の ながさ と サイコロの目。みじかい→ながい で 段差をつける（すごろくへの橋渡し）
const MODES = {
  short: { len: 5, die: 3, label: 'みじかい' },
  long: { len: 10, die: 6, label: 'ながい' },
} as const;
...
export function DiceWalkUnit({ characterId, characterName, onExit }: Props) {
  ...
  const { len: trackLen, die: dieMax } = MODES[mode];
  const [pos, setPos] = useState(0);       // 0 = スタート、1..マス数
  ...
  function handleRoll() {
    ...
    const remain = trackLen - pos;
    const max = Math.min(dieMax, remain);
    const value = 1 + Math.floor(Math.random() * max);
    // ころがる演出のあいだ 目を チラチラ変える
    later(() => setDie(1 + Math.floor(Math.random() * dieMax)), ROLL_MS * 0.4);
    later(() => {
      setDie(value);
  ...
  function reachGoal() {
    ...
    speakJa(`おうちに ついたよ！ ゴール！ ${characterName}、ぜんぶで ${trackLen} ばんまで すすんだね！`, undefined, { rate: SPEAK_RATE });
  ...
  // 難易度を かえる（あそびの とちゅうは かえない。かえたら さいしょから）
  ...
  return (
    <div className="flex h-screen flex-col items-center gap-4 bg-gradient-to-b from-sky-200 via-rose-100 to-amber-50 p-4">
      ...
      {/* 難易度：みじかい(5マス) / ながい(10マス)。サイコロの目も かわる */}
      <div className="flex gap-2">
        {(Object.keys(MODES) as Mode[]).map((m) => (
          <button
            ...
            disabled={busy}
            className={`rounded-full px-4 py-1.5 text-sm font-bold shadow transition-colors ${
              mode === m ? 'bg-rose-500 text-white' : 'bg-white/70 text-rose-600'
            } ${busy ? 'opacity-50' : ''}`}
          >
            {MODES[m].label}（{MODES[m].len}マス）
          </button>
        ))}
      </div>
      ...
      <div ref={trackRef} className="flex w-full max-w-2xl items-end gap-2 overflow-x-auto py-6 px-1">
        {Array.from({ length: trackLen + 1 }).map((_, idx) => {
          const here = pos === idx;
          const isStart = idx === 0;
          const isGoal = idx === trackLen;
          return (
            <div
              key={idx}

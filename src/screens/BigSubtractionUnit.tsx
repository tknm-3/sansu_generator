  ...
  function handlePick(value: number) {
    ...
    if (correct) {
      ...
      if (nextSolved >= QUESTIONS_PER_UNIT) {
        ...
        setTimeout(() => { setExpression('normal'); setProblem(generateBigSubtraction()); setScenario(pickScenario('big-subtraction')); setShowFormula(false); setShowVisual(false); processing.current = false; }, 900);
      }
    } else {
      playSfx('wrong');
      setFeedback('wrong');

...
- **ビンゴすごろくの演出・タイマー・読み上げ(speakJa)を直すときは `bingo-sugoroku-timing` スキルを見る**
  （`timerRef` 使い回しの罠＝「サイコロを振っても動かない」系バグの未然防止）。
...
サーバー側のストリーミングバグ（`stop_reason: tool_use` なのにツール呼び出しが空になる）は、
**ツール引数が巨大・特殊文字てんこ盛り・長いターン**で出やすい。以下のルールで頻度を下げる。

- **Edit は最小範囲で**: 1回の `Edit` で置換するブロックをできるだけ小さく保つ。
  数百行を一括置換しない。
- **新規ファイルは分割書き込み**: 骨組みを先に `Write` → セクションを `Edit` で追記、
  と複数回に分けて書く。100行超を一発で書き込まない。
- **bash heredoc に長文・日本語・引用符を混ぜない**: 長い文字列は `Write`/`Edit` で扱い、
  bash は短いコマンドだけに使う。
- **1ターンのツール呼び出し数を絞る**: 並列可能な呼び出しでも、数が多いと壊れやすい。
  互いに依存しない読み取りは並列可だが、編集は数ステップごとに区切って進捗を報告する。
- **1ファイルずつ変更**: 複数ファイルを同時編集しない。1ファイル変更→ビルド/テスト確認→次へ。
- **「考える」ターンと「編集する」ターンを分ける**: 設計検討は短い計画を提示してから着手し、
  重い思考と編集を同一ターンに詰め込まない。
- **こまめに `git commit`**: ターンが飛んでも被害ゼロにする。きりが良い単位で必ずコミット。

## プルリク（PR）を出すときの方針
- **PRを出す/更新する前に、ベース(origin/main)とのコンフリクトを必ず解消する。手順は
  `pull-request-flow` スキルを見る**（fetch→ドライランmerge→マーカー解消→test+build→push→PR作成）。
